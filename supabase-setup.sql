...
-- Müşteri tablosu için politikalar
...
-- API key tablosu için politikalar
CREATE POLICY "API keys are viewable by authenticated users" ON api_keys
  FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "API keys can be inserted by authenticated users" ON api_keys
  FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "API keys can be updated by authenticated users" ON api_keys
  FOR UPDATE USING (auth.role() = 'authenticated');
